...
Add KubePMIx to docker compose, set following:

- `KUBE_PMIX_NAMESPACE_NAME`, `KUBE_PMIX_JOB_SIZE` - Name and size of the namespace to be created on the startup of KubePMIx
- `PMIX_MCA_ptl_*` - OpenPMIx PTL settings to specify the server's endpoint information. KubePMIx reflects all the OpenPMIx MCA settings as it runs server underneath.
